...
class AutomationEventTest : TestBase() {
    ...
    @Test fun oldSmsActionDisablesRuleWithoutRunningRemainingActions() {
        val event = AutomationEventObject(injector)
        event.trigger = TriggerDummy(injector).instantiate(JSONObject(TriggerConnectorTest().oneItem)) as TriggerConnector
        event.addAction(ActionSMBChange(injector))
        val saved = JSONObject(event.toJSON())
        saved.put("actions", JSONArray().put(JSONObject().put("type", "ActionSendSMS").toString()).put(event.actions[0].toJSON()))

        val restored = AutomationEventObject(injector).fromJSON(saved.toString())

        assertThat(restored.isEnabled).isFalse()
        assertThat(restored.actions).isEmpty()
        restored.isEnabled = true
        assertThat(restored.isEnabled).isFalse()
        val persisted = JSONObject(restored.toJSON())
        assertThat(persisted.getBoolean("enabled")).isFalse()
        assertThat(persisted.getJSONArray("actions").length()).isEqualTo(2)
    }
}
